...
const NavBar = () => {
  ...
  return (
    <div className="navbar">
        ...
        <ul className='nav-links'>
          {
            navLinks.map((link) => (
              <li key={link.id}>
                <a href={`#${link.id}`} onClick={goTo(link.id)}>{link.label}</a>
              </li>
            ))
          }

          <li>
            <a href="/assets/files/cv.pd" className="download-button" download="cv.pdf">RESUME</a>
          </li>



        </ul>
        <button type='button' className={`nav-burger ${open ? "is-open" : ""}`} aria-label='Toggle menu'
          aria-expanded={open}
      ...
      <div ref={pannelRef} id='nav-panel' className="nav-panel">
        {navLinks.map((link) => (
